...
async function seed() {
    ...
    const products = await Promise.all(getProducts().map((p) => db.product.create({ data: p })));
    await Promise.all(getProductImages().map((_a) => {
        var { productName } = _a, otherData = __rest(_a, ["productName"]);
        const product = products.find((p) => p.name === productName);
        if (!product)
            throw new Error(`Product not found: ${productName}`);
        return db.productImage.create({
            data: Object.assign(Object.assign({}, otherData), { product: { connect: { id: product.id } } }),
        });
    }));
    const orders = await Promise.all(getOrders().map((o) => db.order.create({ data: Object.assign(Object.assign({}, o), { totalPrice: 0 }) })));
    for (const order of orders) {
        const items = getOrderItems().filter((i) => i.customerName === order.customerName);
        let total = 0;
